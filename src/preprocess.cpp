...
Preprocess::Preprocess()
    : feature_enabled(0), lidar_type(AVIA), blind(0.01),
      point_filter_num(1) { // 是否提取特征、雷达类型、盲区距离、点云滤波点数
  inf_bound = 10; // 用于判断边缘的距离上限（提取特征）
  N_SCANS = 6;    // 雷达线数
  group_size = 8; // 每组点的数量下限（提取特征）
  disA = 0.01;    // 用于计算每组允许的最远距离（提取特征）
  disA = 0.1; // note: B? disB 用于计算每组允许的最远距离（提取特征）
  p2l_ratio = 225; // 点到线的距离比（提取特征）
  limit_maxmid = 6.25; // 组内头部与中间距离差的比上限（提取特征）
  limit_midmin = 6.25; // 组内中间与尾部距离差的比上限（提取特征）
  limit_maxmin = 3.24; // 组内头部与尾部距离差的比上限（提取特征）
  jump_up_limit = 170.0; // 判断激光射线与邻点方向的夹角（提取特征）
  jump_down_limit = 8.0; // 判断激光射线与邻点方向的夹角（提取特征）
  cos160 = 160.0; // 判断两邻点向量之间的夹角（提取特征）
  edgea = 2; // 距离差的倍数，用于判断边缘跳跃点（提取特征）
  edgeb = 0.1; // 距离差的差，用于判断边缘跳跃点（提取特征）
  smallp_intersect = 172.5; // 两邻点向量之间的夹角上限（提取特征）
  smallp_ratio = 1.2; // 两邻点距离差的比的上限（提取特征）
  given_offset_time = false; // 是否给定偏移时间，用于vlp等无时间偏移量的雷达
  ...
  if (feature_enabled) {
    ...
    for (uint i = 0; i < plsize; i++) {
      if ((msg->points[i].line <
           N_SCANS)) // && ((msg->points[i].tag & 0x30) == 0x10))
      {
        valid_num++;
...
void Preprocess::Pandar128_handler(
    const sensor_msgs::PointCloud2::ConstPtr &msg) {
  pl_surf.clear();
...
void Preprocess::robosense_handler(
  ...
  for (int i = 0; i < plsize; ++i) {
    ...
    const bool is_valid = (dist_sqr >= blind_sqr) && !std::isnan(x) &&
...
void Preprocess::give_feature(pcl::PointCloud<PointType> &pl,
  ...
  for (uint i = head + 1; i < plsize2; i++) {
    if (types[i].range < blind_sqr || types[i - 1].range < blind_sqr ||
        types[i + 1].range < blind_sqr) {
      continue;
    }
