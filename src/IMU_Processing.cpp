...
ImuProcess::ImuProcess()
    : Eye3d(M3D::Identity()), Zero3d(0, 0, 0), b_first_frame(true),
      imu_need_init(true) { // 单位阵、零向量、是否第一帧、是否需要初始化
  init_iter_num = 1;                       // 初始化迭代次数
  cov_acc = V3D(0.1, 0.1, 0.1);            // 加速度协方差
  cov_gyr = V3D(0.1, 0.1, 0.1);            // 角速度协方差
  cov_bias_gyr = V3D(0.1, 0.1, 0.1);       // 角速度偏置协方差
  cov_bias_acc = V3D(0.1, 0.1, 0.1);       // 加速度偏置协方差
  cov_inv_expo = 0.2;                      // 逆曝光时间协方差
  mean_acc = V3D(0, 0, -1.0);              // 初始平均加速度
  mean_gyr = V3D(0, 0, 0);                 // 初始平均角速度
  angvel_last = Zero3d;                    // 上一时刻角速度
  acc_s_last = Zero3d;                     // 上一时刻加速度
  Lid_offset_to_IMU = Zero3d;              // 雷达到IMU的平移
  Lid_rot_to_IMU = Eye3d;                  // 雷达到IMU的旋转
  last_imu.reset(new sensor_msgs::Imu());  // 上一时刻IMU数据重置
  cur_pcl_un_.reset(new PointCloudXYZI()); // 当前无畸变点云重置
}
